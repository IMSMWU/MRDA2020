...
### Load in a Qualtrics survey via package "qualtRics"? {-}
...
head(qualtrics,3)
```
Current column names are not much helpful in identyfing questions from the questionnaire. In order to name columns after corresponding question, the function `label_to_colnames()` from package `sjlabelled` can help. 
...
p <- ggplot(data=mlc_visualisation, 
             ...
             geom_bar(stat='identity') + theme(axis.text.x = element_blank()) + labs(title = 'The period of the day you prefer watching movies?')+xlab(label = " ")
p
```
