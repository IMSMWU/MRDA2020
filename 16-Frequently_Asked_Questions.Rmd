...
colnames(Importance) <- c("Stability","Education","Healthcare","Culture","Name")
```
...
Additionally, we can caluclate mean values and compare them with `ggParir()`:

```{r,warning=FALSE,message=FALSE}
(Ranking_mean <-Radar_chart_new %>%
  filter(Name=="Ranking")%>% # Addressing only rows with "Ranking" in column "Name" 
  summarize(Avg.Stability=mean(Stability), # computing mean for every column
            Avg.Education=mean(Education),
            Avg.Healthcare=mean(Healthcare),
            Avg.Culture=mean(Culture)) %>% 
  add_rownames(var = "Name") %>% # adding a row name
  mutate(Name=as.factor("Ranking"))) # Changing the name value from 1 to "Ranking" and turning it to factor
```

```{r,warning=FALSE,message=FALSE}
(Importance_mean <-Radar_chart_new %>%
  filter(Name=="Importance")%>% # Addressing only rows with "Importance" in column "Name" 
  summarize(Avg.Stability=mean(Stability), # computing mean for every column
            Avg.Education=mean(Education),
            Avg.Healthcare=mean(Healthcare),
            Avg.Culture=mean(Culture)) %>% 
  add_rownames(var = "Name") %>% # adding a row name
  mutate(Name=as.factor("Importance"))) # Changing the name value from 1 to "Importance" and turning it to factor
```
...
## How to collapse/recode categories?
...
For this purpose we will create a data frame at the spot:
